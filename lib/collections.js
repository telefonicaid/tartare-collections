...
var request = require('request'),
    ...
    tartareHttp = require('tartare-util').http;
...
var ApiCollectionsGroup = function ApiCollectionsGroup(config) {
  ...
  var Collection = function Collection(collectionPath) {
    ...
    var baseUrl = config.baseUrl + (config.baseUrl.slice(-1) === '/' ? '' : '/') +
                  (collectionPath.slice(-1) === '/' ? collectionPath.slice(0, -1) : collectionPath);
    ...
    var _apiRequest = function _apiRequest(method, criteria, body, opt, cb) {
      ...
      request(opt, function handleResponse(err, res) {
        if (err) {
          //console.log('err:', err);
          return cb(err);
        }
  ...
  return {
     ...
     */
    createCollection: function createCollection(path) {
